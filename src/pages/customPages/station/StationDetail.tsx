...
const StationDetail = () => {
  ...
  const {
    data: stationProgramData,
    isLoading: stationProgramLoading,
    error: stationProgramError,
    isSuccess: stationSuccess,
  }: any = useProgramByStationQuery(paramsId);

  if (isLoading || stationProgramLoading) return <CircularProgress />;
  if (error || stationProgramError)
    return <Typography variant="h4">Something Went Wrong</Typography>;
  if (stationSuccess) {
    programData = stationProgramData.data.programs;
  }
  stationData = data;
    ...
    <Box>
      ...
      <Grid container>
        <Grid item lg={12} md={12} sm={12} xs={12}>
          <Typography variant="h3">
            {isLoading ? <Skeleton width="50%" /> : stationData && stationData.data.name}
          </Typography>
        </Grid>
        <Grid item lg={12} md={12} sm={12} xs={12} sx={{ ml: 4 }}>
          <Typography variant="inherit">
            ...
            {isLoading ? (
              ...
              stationData && stationData.data.description
            )}{' '}
          </Typography>
        </Grid>
      </Grid>
      <Grid container sx={{ mt: 5 }}>
        <Grid item lg={12} md={12} sm={12} xs={12}>
          ...
          <ProgramByStationComponent programData={programData} />
        </Grid>
      </Grid>
    </Box>
  );
